...
// Outbound voice TwiML (connect stream)
...
router.post("/outbound-status", async (req, res) => {
  try {
    ...
    if (callLog) {
      ...
      if (duration != null) callLog.duration = duration;
...
router.post("/transfer/:callSid", async (req, res) => {
  try {
    ...
    if (!callLog?.campaign) return res.status(404).json({ error: "CallLog/Campaign not found" });
    ...
    if (!enabled) return res.status(400).json({ error: "Transfer is disabled for this campaign" });
